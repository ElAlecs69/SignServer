...
TRUST_CA_FILE = os.getenv("TRUST_CA_FILE", "/etc/signserver/trust/trust-chain.pem")
...
def trust_ca_file() -> str:
    if not os.path.isfile(TRUST_CA_FILE):
        raise HTTPException(
            503,
            "No está disponible la cadena de confianza configurada en "
            f"{TRUST_CA_FILE}.",
        )
    return TRUST_CA_FILE
...
def validate_xml(req: ValidateRequest):
    ...
    try:
        proc = run(["xmlsec1", "--verify", "--trusted-pem", trust_ca_file(), path])
    except FileNotFoundError:
        cleanup(path)
